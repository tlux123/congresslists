...
function createEditorialSection({ key, eyebrow, title, items, textLayout = false }) {
  return {
    key,
    eyebrow,
    title,
    ...
    items: items.map((item, index) => ({
      rank: index + 1,
      name: item.name,
      photoUrl: textLayout ? null : (item.photoUrl || null),
      noPhoto: textLayout,
...
function createEditorialSections(peopleByName) {
  return [
    ...
    createEditorialSection({
      key: "doctorsInPolitics",
      eyebrow: "Medical field",
      title: "Physicians Who've Entered the Arena",
      textLayout: true,
      items: [
        { name: "Amish Shah", meta: "Emergency physician · Arizona" },
        { name: "Kelly Morrison", meta: "OB/GYN · Minnesota" },
        { name: "Maxine Dexter", meta: "Pulmonary & critical care · Oregon" },
        { name: "Kristin Lyerly", meta: "OB/GYN · Wisconsin" },
        { name: "Herb Conaway", meta: "Internal medicine · New Jersey" },
        { name: "Mike Kennedy", meta: "Family physician · Utah" },
        { name: "Bob Onder", meta: "Allergist & immunologist · Missouri" },
        { name: "Tim Peck", meta: "Emergency physician · Indiana" },
      ],
    }),
  ];
}
